...
--------------------- 제약조건 : 테이블,데이터를 저장하는 규약,제약 조건 (데이터 무결성)
CREATE TABLE TBL_EX (
...
INSERT INTO TBL_EX (LOGIN_ID, LOGIN_PW, TEL)
VALUES('BBBB', '2345', NULL);
-- ALTER TABLE TBL_EX MODIFY TEL NOT NULL; 		-- TEL에 이미 NULL값이 있어서 제약조건 수정 불가
UPDATE TBL_EX SET TEL = '010-****-9244' WHERE LOGIN_ID = 'AAAA';
-- UNIQUE
...
SELECT * FROM EMP;
SELECT * FROM DEPT;
-- 1. EMP 테이블에서 사원번호가 7521인 사원의 직업과 같고, 사원번호 7934인 사원의 급여보다 많은 사원의 사번, 이름, 직업, 급여 출력
SELECT DEPTNO, ENAME, JOB, SAL 
FROM EMP 
WHERE JOB = (SELECT JOB FROM EMP WHERE EMPNO = 7521) AND SAL > (SELECT SAL FROM EMP WHERE EMPNO = 7934);

-- 2. 직업별로 최소 급여를 받는 사원의 정보를 사원번호, 이름, 업무, 부서명으로 출력(직업별로 내림차순)
...
-- 3. 각 사원별 커미션이 0 또는 null이고 부서 위치가 'GO'로 끝나는 사원의 정보를 사원번호, 사원이름, 커미션, 부서번호, 부서명을 출력 (보너스가 NULL이면 0으로 출력)
SELECT EMPNO, ENAME, NVL(COMM, 0), E.DEPTNO, DNAME
FROM EMP E LEFT OUTER JOIN DEPT D ON E.DEPTNO = D.DEPTNO
